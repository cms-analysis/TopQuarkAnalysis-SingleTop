...
#G.Benelli
#loosening the electron Et cut (not touching eta).
eleTightCut = cms.string("et>15  && abs(eta)<2.5  & (gsfTrack().trackerExpectedHitsInner.numberOfHits == 0) & dB < 0.02 & ( abs(superCluster.eta)> 1.5660 || abs(superCluster.eta)<1.4442)")
...
#G.Benelli
#Loosening the muon eta cut, removing the numberOfHits cut and the pixelLayer cut from muons
muTightCut = cms.string("pt > 20 & isGlobalMuon && isTrackerMuon & abs(eta) < 2.4 && numberOfMatches() > 1  && muonID('GlobalMuonPromptTight') > 0 & dB < 0.02 ")
...
#G.Benelli
#Loosening the jet Pt cut for synching and systematics studies:
jetLooseCut = cms.string("numberOfDaughters()>1 & pt()> 15 && abs(eta())<5 & ((abs(eta())>2.4) || ( chargedHadronEnergyFraction() > 0 & chargedMultiplicity()>0 & neutralEmEnergyFraction() < 0.99 & neutralHadronEnergyFraction() < 0.99 & chargedEmEnergyFraction()<0.99))")
